"""CodeAct Sandbox - Manages Daytona sandbox for code execution."""
...
from .tool_indexer import create_tool_index
...
class CodeActSandbox:
    """Manages Daytona sandbox for CodeAct pattern execution."""

    def __init__(self, config: CoreConfig, mcp_registry: MCPRegistry):
        """Initialize CodeAct sandbox.
        ...
        logger.info("Initialized CodeActSandbox")
    ...
    def _create_snapshot_image(self) -> Image:
        ...
        dependencies = [
            # Core
            "mcp", "pandas", "requests", "aiohttp", "httpx",
            # Data science
        ...
        # Get MCP server npm packages from config
        mcp_packages = []
    ...
    def _get_snapshot_hash(self) -> str:
        ...
        config_data = {
            ...
            'apt_packages': ["curl", "nodejs", "ripgrep"],  # Include apt packages in hash
        }

        config_str = json.dumps(config_data, sort_keys=True)
        return hashlib.sha256(config_str.encode()).hexdigest()[:8]

    async def _ensure_snapshot(self) -> Optional[str]:
        ...
        base_name = self.config.daytona.snapshot_name or "codeact-base"
    ...
    async def _setup_workspace(self) -> None:
        ...
        for directory in directories:
            try:
                # Create directory using process.exec (more reliable)
    ...
    async def _install_dependencies(self) -> None:
        ...
        install_cmd = f"pip install -q {' '.join(dependencies)}"

        try:
            result = await self._run_sync(self.sandbox.process.exec, install_cmd)
    ...
    async def _install_tool_modules(self) -> None:
        ...
        # Generate and upload MCP client module first
        ...
        logger.info("MCP client module installed", path=mcp_client_path)
        ...
        for server_name, tools in tools_by_server.items():
            ...
            # Write to sandbox using absolute path
            ...
            for tool in tools:
                ...
                doc_path = f"{work_dir}/tools/docs/{tool.name}.md"
                await self._run_sync(
                    self.sandbox.fs.upload_file,
                    doc.encode('utf-8'),
                    doc_path
        ...
        # Generate tool index for semantic search
        all_tools = []
        for server_name, tools in tools_by_server.items():
            all_tools.extend(tools)

        if all_tools:
            indexer = create_tool_index(all_tools)
            index_data = indexer.generate_index()
            index_json = json.dumps(index_data, indent=2)
            index_path = f"{work_dir}/tools/tool_index.json"
            await self._run_sync(
                self.sandbox.fs.upload_file,
            ...
            logger.info(
                "Tool index generated",
                path=index_path,
                tool_count=len(all_tools),
                categories=list(indexer.categories.keys())
            )
    ...
    async def execute(
        ...
        try:
            ...
            files_before = await self._list_result_files_async()
            ...
            files_after = await self._list_result_files_async()
...
"""
            ...
            script_path = f"/tmp/bash_exec_{int(time.time())}.sh"
            await self._run_sync(
                self.sandbox.fs.upload_file,
...
"""
    ...
    def _list_result_files(self) -> List[str]:
        ...
        """
        try:
            file_infos = self.sandbox.fs.list_files("results")
            if not file_infos:
                return []
    ...
    async def _list_result_files_async(self) -> List[str]:
        """List files in the results directory asynchronously.
    ...
    def download_file_bytes(self, filepath: str) -> Optional[bytes]:
    ...
    def validate_path(self, filepath: str) -> bool:
        ...
        # Normalize the path for sandbox (don't use resolve() as it resolves locally)
        # Just normalize . and .. components without resolving against local filesystem
        normalized_path = str(Path(filepath))
        if not normalized_path.startswith('/'):
            # Relative paths should be prefixed with work directory
            work_dir = getattr(self, '_work_dir', '/home/daytona')
            normalized_path = f"{work_dir}/{normalized_path}"
    ...
    def search_files(self, pattern: str, directory: str = ".", exclude: Optional[List[str]] = None) -> List[str]:
    ...
    def glob_files(self, pattern: str, path: str = ".") -> List[str]:
        ...
        try:
            ...
            if path == ".":
                search_path = "/home/daytona"
            elif not path.startswith("/"):
                search_path = f"/home/daytona/{path}"
            else:
                search_path = path
...
'''
    ...
    def grep_content(
        ...
        lines_context: Optional[int] = None,
    ...
    def read_file_range(self, file_path: str, offset: int = 0, limit: int = 2000) -> Optional[str]:
